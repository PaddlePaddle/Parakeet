...
from data import Baker, collate_baker_examples


class Experiment(ExperimentBase):
    def compute_losses(self, inputs, outputs):
        _, _, mel_targets, _, _, stop_tokens = inputs
    ...
    def train_batch(self):
        ...
        self.model.train()
        texts, tones, mels, text_lens, output_lens, stop_tokens = batch
        outputs = self.model(texts, mels, text_lens, output_lens, tones=tones)
    ...
    def setup_dataloader(self):
        args = self.args
        config = self.config
        ljspeech_dataset = Baker(args.data)

        valid_set, train_set = dataset.split(ljspeech_dataset,
                                             config.data.valid_size)
        batch_fn = collate_baker_examples
    ...
    def eval(self):
        ...
        for i, batch in enumerate(self.valid_loader):
            texts, tones, mels, text_lens, output_lens, stop_tokens = batch
            outputs = self.model.infer(texts, stop_threshold=0.1, max_decoder_steps=1000, tones=tones)
            ...
            np.save(self.output_dir / ("sentence_{}".format(i)), outputs['mel_output'][0].numpy().T)
            print(self.output_dir / ("sentence_{}".format(i)))
